...
// const DiscoverUsersScreen = ({ navigation }) => {
//   // Estados
...
import AsyncStorage from '@react-native-async-storage/async-storage';
import { getUserById, getUsersByLanguage, toggleActiveToChat } from '../apiServices';
...
const DiscoverUsersScreen = ({ navigation }) => {
  ...
  const [hasMore, setHasMore] = useState(true);
  const [lastVisible, setLastVisible] = useState(null);
  const [isActiveToChat, setIsActiveToChat] = useState(true);
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("Usuario actual:", {
          ...
          activeToChat: user.activeToChat
        });
  
        setCurrentUser(user);
        setIsActiveToChat(user.activeToChat !== false); // Por defecto true si no está definido
        await loadPartners(user.practicingLanguage, true);
      } catch (err) {
        console.error("Error:", err);
        setError(err.message || "Error al cargar datos");
  ...
  const loadPartners = async (language, isRefreshing = false) => {
    try {
      ...
      const { users, lastVisible: newLastVisible } = await getUsersByLanguage({
        nativeLanguage: language,
        activeToChat: true, // Solo mostrar usuarios activos para chatear
        limit: 10,
        lastVisibleDoc: isRefreshing ? null : lastVisible
      });
  ...
  const handleToggleActiveToChat = async (value) => {
    try {
      const userId = await AsyncStorage.getItem("key");
      if (!userId) return;
      
      setIsActiveToChat(value);
      await toggleActiveToChat(userId, value);
      
      // Si el usuario desactiva el chat, refrescamos la lista
      if (!value) {
        handleRefresh();
      }
    } catch (error) {
      console.error("Error al cambiar el estado de activeToChat:", error);
      setIsActiveToChat(!value); // Revertir el cambio si hay error
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <View style={styles.switchContainer}>
          <Text style={styles.switchText}>Activo para chatear</Text>
          <Switch
            value={isActiveToChat}
            onValueChange={handleToggleActiveToChat}
            trackColor={{ false: "#767577", true: "#000000" }}
            thumbColor={isActiveToChat ? "#ffffff" : "#f4f3f4"}
